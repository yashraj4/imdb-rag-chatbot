...
export default function Chat() {
  const { messages, input, handleInputChange, handleSubmit, isLoading } = useChat({
    onError: (err) => {
      alert(`Chat error: ${err.message || 'Check server connection or API key configurations.'}`);
    }
  });
      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-6 max-w-3xl mx-auto w-full pb-32">
        ...
        {isLoading && (
          <div className="flex gap-4 items-end justify-start">
            <div className="w-8 h-8 rounded-full bg-violet-600/20 border border-violet-500/30 flex items-center justify-center shrink-0 animate-pulse">
